...
public class Functions {
	...
	public static void embargarCuentasAhorroJudicialLimite(ArrayList<Persona> personas,ArrayList<Cuenta> cuentas,BigDecimal limite) 
	{	
		BigDecimal montoPorEmbargar;
		for(Persona persona:personas) {
			if(persona.getMontoAEmbargar().compareTo(persona.getMontoEmbargado())==1){
				BigDecimal monto=persona.getMontoAEmbargar().subtract(persona.getMontoEmbargado());
				if(monto.compareTo(new BigDecimal(0))>0){
					montoPorEmbargar=monto;
				}else{
					montoPorEmbargar=persona.getMontoAEmbargar();
				}
				BigDecimal saldoExcedente = cuentas.stream().filter(c -> c.getIdPersona().equalsIgnoreCase(persona.getIdPersona()))
					    .map(Cuenta::getSaldoCuentaFecha)
						...
						for(int i=0;i<cuentas.size()&&montoAEmbargar.compareTo(new BigDecimal(0))>0;i++){
							...
							if(cuenta.getIdPersona().equals(persona.getIdPersona())) {	
								if(montoAEmbargar.compareTo(cuenta.getSaldoCuentaFecha())>0){	
									...
									persona.setMontoEmbargado(cuenta.getSaldoCuentaFecha().add(persona.getMontoEmbargado()));
	...
	public static void embargarCuentasAhorroCoactivoLimite(ArrayList<Persona> personas,ArrayList<Cuenta> cuentas,BigDecimal limite) 
	{	
		for(Persona persona:personas) {
			BigDecimal montoAEmbargar;
			if(persona.getMontoAEmbargar().compareTo(persona.getMontoEmbargado())==1){
				...
				Cuenta cuentaProtegida = cuentas.stream().filter(c -> c.getIdPersona().equalsIgnoreCase(persona.getIdPersona()))
	...
	public static void embargarCuentasLimite(ArrayList<Persona> personas,ArrayList<Cuenta> cuentas,ArrayList<String> reglas,
											BigDecimal limite) 
	{	
		for(Persona persona:personas){		
			if(persona.getMontoAEmbargar().compareTo(persona.getMontoEmbargado())==1){
					...
					if(cuenta.getIdPersona().equals(persona.getIdPersona())) {
						if(cuenta.getSaldoCuentaFecha().compareTo(limite)>0) {
							...
							if(montoAEmbargar.compareTo(monto)>0){	
								...
								for(String regla:reglas) {cuenta.getReglas().add(regla);}
